...
class DatasetGenerator(Dataset):
    ...
    def __init__(self, num_nodes, num_samples, name='dataset', random_seed=111):
        ...
        for _ in tqdm(range(num_samples)):
            ...
            if 'DT' in name:
                x = torch.cat((-torch.ones(2, 1), x), dim=1)
...
class GraphEmbedding(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for i in range(seq_len):
            ...
            embedding = torch.bmm(inputs[:, :, :, i], embedding_1)
            if self.use_sdne:
                # embedding = [batch, 1, ebd_size_2]
                embedding = torch.bmm(embedding, embedding_2)
                # embedding = [batch, 1, ebd_size_3]
                embedding = torch.bmm(embedding, embedding_3)
            embedded.append(embedding)
        embedded = torch.cat(tuple(embedded), 1)
        return embedded


def reward(problems, sample_solutions, use_cuda=True, name='reward'):
    """
    Args:
        problems: tensor [batch_size, seq_len, 2]
        sample_solutions: list [batch_size * [n_nodes] ]
    """
    ...
    if 'DT' in name:
        ...
        percentages = Variable(torch.zeros([batch_size]))

        for i in range(batch_size):
            problem = problems[i, :, :].squeeze(0)
            problem = problem.cpu().numpy()
            sample_solution = sample_solutions[i]
            solution = Delaunay(problem).simplices
            n_nodes = len(sample_solution)

            sample_edges = []
            if n_nodes > 1:
                for j in range(n_nodes-1):
                    sample_edge = frozenset([sample_solution[j], sample_solution[j+1]])
                    if not (sample_edge in sample_edges):
            ...
            for tri in solution:
                for j in range(2):
                    edge = frozenset([tri[j], tri[j+1]])
                    if not (edge in edges):
                        edges.append(frozenset(edge))
            edges = set(edges)

            percentages[i] = (1. - (len(sample_edges & edges) / len(sample_edges | edges))) * 100

        rewards = percentages

        if use_cuda:
            rewards = percentages.cuda()
...
class CombinatorialRL(nn.Module):
    ...
    def forward(self, inputs, is_training, add_noise):
        """
        Args:
            inputs: [batch_size, input_size=2, seq_len]
        """
        ...
        probs, action_idxs, stop_dict, loop_idx = self.actor(inputs, is_training=is_training, add_noise=add_noise)
        print(loop_idx)
        print(stop_dict)

        # inputs = [batch_size, seq_len, input_size=2]
        inputs = inputs.transpose(1, 2)
        actions = [[action_idxs[j].cpu().numpy()[i] for j in range(stop_dict[i])] for i in range(batch_size)]
        for action in actions:
            print(action)

        R = self.reward(inputs, actions, self.use_cuda, self.name)
    ...
    def train_and_validate(self, args, n_epochs):
        ...
        for epoch in range(self.start_epoch, n_epochs):
            ...
            for batch_id, sample_batch in data_loaders:
                ...
                print(critic_exp_mvg_avg.detach())
                self.advantage.append(advantage.mean().detach())
                print(advantage.mean().detach())
